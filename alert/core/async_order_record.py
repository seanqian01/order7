...
def update_order_details_async(order_record_id):
    ...
    try:
        ...
        # 如果订单已经是取消状态，直接跳过API查询
        if order_record.status == 'CANCELLED':
            logger.info(f"订单 {order_record.order_id} 已取消，跳过API查询")
            return
        ...
        if order_details and order_details["status"] == "success":
            ...
            if "oid" in order_details and order_details["oid"] is not None:
                ...
                logger.info(f"获取到订单渠道ID: {mask_sensitive_info(order_details['oid'])}")
            else:
                missing_fields.append("oid")
            ...
            if "filled_time" in order_details and order_details["filled_time"] is not None:
                ...
                try:
                    ...
                    if order_details["filled_time"] > 10000000000:  # 判断是否为毫秒时间戳
                        # 将毫秒转换为秒
                ...
                try:
                    ...
                    try:
                        ...
                        with connection.cursor() as cursor:
                            ...
                            if "oid" in order_details and order_details["oid"] is not None:
                                update_fields.append("oid = %s")
                                params.append(str(order_details["oid"]))
            ...
            # 对于已取消的订单，如果API返回unknownOid，这是正常情况
            if order_record.status == 'CANCELLED' and error_msg == 'API返回unknownOid状态，订单可能已过期或不存在':
                logger.info(f"订单 {order_record.order_id} 已取消且API中不存在，这是正常情况")
            else:
...
def get_order_details(trader, symbol, order_id, order_status):
    ...
    try:
        ...
        # 如果订单状态为已取消，直接跳过API查询
        if order_status == 'CANCELLED':
            logger.info(f"订单 {order_id} 状态为已取消，跳过API查询")
            return {
                "status": "success",
                "oid": order_id,  # 使用原始订单ID作为渠道ID
                "fee": Decimal('0'),
                "filled_time": None,
            ...
            for fill in user_fills:
                if str(fill.get("oid")) == str(order_id):
                    ...
                    return {
                        ...
                        "oid": str(oid),
                        "fee": Decimal(str(fee)),
                        "filled_time": datetime.fromtimestamp(filled_time / 1000),
                        "source": "user_fills"
                    }
            
            logger.info(f"在成交记录中未找到订单ID为 {order_id} 的记录")
            
            # 如果订单状态为已取消，并且在成交记录中未找到，可以直接返回
            if order_status == 'CANCELLED':
                logger.info(f"订单 {order_id} 已取消且未在成交记录中找到，可能未成交")
                return {
                    "status": "success",
                    "oid": order_id,  # 使用原始订单ID作为渠道ID
                    "fee": Decimal('0'),
        ...
        # 首先尝试从状态中获取 oid，不管订单状态如何
        if "oid" in status:
            result["oid"] = str(status["oid"])
            logger.info(f"直接从状态中获取到订单的oid: {mask_sensitive_info(result['oid'])}")
        ...
        if "filled" in status:
            ...
            # 检查oid字段
            if "oid" in filled_info:
                result["oid"] = str(filled_info["oid"])
                logger.info(f"成功获取已成交订单的oid: {mask_sensitive_info(result['oid'])}")
            elif not result.get("oid"):
                logger.warning(f"已成交订单中没有oid字段: {filled_info}")
            ...
            if "sz" in filled_info:
                ...
                logger.warning(f"已成交订单中没有sz字段: {filled_info}")
            ...
            if "fee" in filled_info:
                ...
                logger.warning(f"已成交订单中没有fee字段: {filled_info}")
                
            # 检查time字段（成交时间）
            if "time" in filled_info:
                timestamp_ms = int(filled_info["time"])
                # 将毫秒时间戳转换为datetime对象
                if timestamp_ms > 10000000000:  # 判断是否为毫秒时间戳
                    seconds_timestamp = timestamp_ms / 1000
                else:
                    seconds_timestamp = timestamp_ms  # 已经是秒级时间戳
                
                filled_datetime = datetime.fromtimestamp(seconds_timestamp)
                result["filled_time"] = filled_datetime
                logger.info(f"成功获取已成交订单的成交时间: {result['filled_time']}")
            else:
                logger.warning(f"已成交订单中没有time字段: {filled_info}")
                
            logger.info(f"成功解析已成交订单信息: oid={mask_sensitive_info(result.get('oid'))}, fee={result.get('fee')}, filled_time={result.get('filled_time')}")
        # 如果订单挂单中，获取挂单信息
            ...
            # 检查oid字段
            if "oid" in resting_info:
                result["oid"] = str(resting_info["oid"])
                logger.info(f"成功获取挂单中订单的oid: {mask_sensitive_info(result['oid'])}")
            elif not result.get("oid"):
                logger.warning(f"挂单中订单没有oid字段: {resting_info}")
                
            logger.info(f"成功解析挂单中订单信息: oid={mask_sensitive_info(result.get('oid'))}")
        # 如果订单已取消，获取取消信息
            ...
            # 检查oid字段
            if "oid" in canceled_info:
                result["oid"] = str(canceled_info["oid"])
                logger.info(f"成功获取已取消订单的oid: {mask_sensitive_info(result['oid'])}")
            elif not result.get("oid"):
                logger.warning(f"已取消订单没有oid字段: {canceled_info}")
                
            logger.info(f"成功解析已取消订单信息: oid={mask_sensitive_info(result.get('oid'))}")
